...
On Windows, Practice Room also checks the Codex desktop installation and the usual npm folder, even when the launcher cannot find `codex` on PATH. It selects an installed supported version; it does not downgrade another installation. For a custom location, set `PRACTICE_ROOM_CODEX` to the absolute executable path before launching. Connection status and setup help are available in Settings, the coach panel, and repairs. A desktop login is separate from this app's ChatGPT sign-in.

After updating Practice Room, close the old app terminal and reopen **Start Study.cmd**. The launcher refreshes dependencies when their specification changes and rebuilds changed interface assets. An older running server is detected and asks for a restart.
