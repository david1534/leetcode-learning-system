...
Browser actions run in order and read revisions when dispatched. Older state responses cannot restore a completed session. Autosave recognizes its own pending candidate when a poll arrives before the save response, and browser saves include their exercise ID. Keyboard sending obeys the same availability rules as the Send button, a new composer draft survives a delayed acknowledgement, and sending waits for assessment conversion. Practice panel tabs support arrow, Home, and End keys.
