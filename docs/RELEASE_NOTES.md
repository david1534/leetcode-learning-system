...
A private, owned App Server process, running outside the curriculum checkout, embeds Codex through ChatGPT sign-in. The verified adapter supports CLI 0.153.4 and 0.154.0-alpha.6.2 and checks its restrictive configuration before use. Windows discovery checks the command path, the standalone install location, and the newest CLI bundled with Codex Desktop. It does not copy credentials, alter global settings, enable execution tools, use API/provider fallback, buy credits, or redeem resets. Unknown/exhausted allowance prevents new turns; at 20% remaining, automatic checkpoints stop while manual questions remain available. Allowance is shared with other Codex activity and controlled by OpenAI.

The coach panel now keeps allowance details behind a compact status icon, opens preferences in an unclipped panel, and uses a percentage progress bar instead of a jumpy elapsed-time counter.
